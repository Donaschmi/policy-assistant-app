        ...
        <form class="w-full max-w-lg" @submit.prevent="submit">
            <div class="flex flex-wrap -mx-3 mb-6" v-if="currentTab==='event'">
                <div class="w-full px-3 mb-6 md:mb-0">
                    ...
                    <div class="relative">
                        <select v-model="event" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state">
                            <option disabled value="">Choisissez</option>
                            <option v-for="event in events" :value="event">{{ event.sentence }}</option>
                        </select>
                    </div>
                </div>
            </div>
            <div class="flex flex-wrap -mx-3 mb-6" v-if="currentTab==='action'">
                <div class="w-full px-3 mb-6 md:mb-0">
                    ...
                    <jet-dialog-modal :show="showCreateActor" @close="showCreateActor = false">
                        <template #content>
                            <create-actor-form :tenant="tenant" :actor_types="actor_types" :actions="actions"/>
                        </template>
                    </jet-dialog-modal>
                </div>
            </div>
                ...
                <div class="w-full md:w-2/5 px-3 mb-6 md:mb-0">
                    ...
                    <div class="relative">
                        <select v-model="currentAction" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state">
                            <option disabled value="">Choisissez</option>
                            <option v-for="action in actions" :value="action">{{ action.sentence }}</option>
                        </select>
                    </div>
                </div>
                <div class="w-full md:w-2/5 px-3 mb-6 md:mb-0">
            ...
            <div class="flex flex-wrap -mx-3 mb-2">
                ...
                <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
                    <jet-button v-if="currentTab === 'event' && this.event" @click.prevent="currentTab = 'action'">
                        Suivant
                    </jet-button>
                </div>
                <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
